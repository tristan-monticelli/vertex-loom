...
ValidationReport validate_entity(const ProjectManifest&, const EntityDefinition& a) {
    ...
    if (a.document.schema_version != current_entity_schema_version)
        error(r.errors, ErrorCode::unsupported_schema_version, "schemaVersion",
              "only entity schema version 4 is supported");
...
EntityResult parse_entity(const ProjectManifest& m, std::string_view s) {
    ...
    if (ns == j.end() || !ns->is_array()) error(r.errors, ErrorCode::invalid_asset, "nodes", "expected an array");
        ...
        if (source_schema >= 4) {
            boolean(x, "visible", n.visible, r.errors);
            boolean(x, "locked", n.locked, r.errors);
        }
    ...
    if (source_schema >= 1 && source_schema < current_entity_schema_version)
...
EntityResult publish_entity(const std::filesystem::path& root,const ProjectManifest& m,const EntityDefinition& a){EntityResult r;auto migrated=a;if(migrated.document.schema_version>=1&&migrated.document.schema_version<current_entity_schema_version)migrated.document.schema_version=current_entity_schema_version;auto v=validate_entity(m,migrated);if(!v.ok()){r.errors=std::move(v.errors);return r;}auto p=entity_document_path(m,migrated.document.id);auto s=save_document_atomic(root,p,serialize_entity(migrated),[&](std::string_view x){return parse_validation(m,x);});if(!s.ok()){r.errors=std::move(s.errors);return r;}return load_entity(root,m,p);}
}
