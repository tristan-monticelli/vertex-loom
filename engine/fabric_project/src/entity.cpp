...
std::string serialize_entity(const EntityDefinition& a){Json j={{"schemaVersion",a.document.schema_version},{"type",a.document.type},{"id",a.document.id.value},{"name",a.document.name},{"nodes",Json::array()}};for(const auto& n:a.nodes){Json d={{"kind",std::string(to_string(n.drawable.kind))}};if(n.drawable.resource)d["resource"]=ref_json(*n.drawable.resource);if(n.drawable.material)d["material"]=ref_json(*n.drawable.material);j["nodes"].push_back({{"id",n.id},{"name",n.name},{"transform",transform(n.transform)},{"zOrder",n.z_order},{"drawable",d}});if(n.parent)j["nodes"].back()["parent"]=*n.parent;}return j.dump(2)+"\n";}
EntityResult parse_entity(const ProjectManifest& m,std::string_view s){EntityResult r;Json j;try{j=Json::parse(s);}catch(...){error(r.errors,ErrorCode::invalid_json,"entity","cannot parse entity JSON");return r;}if(!j.is_object()){error(r.errors,ErrorCode::invalid_asset,"entity","top-level value must be an object");return r;}EntityDefinition a;auto schema=j.find("schemaVersion");if(schema==j.end()||!schema->is_number_unsigned())error(r.errors,ErrorCode::invalid_asset,"schemaVersion","expected an unsigned integer");else a.document.schema_version=schema->get<std::uint32_t>();text(j,"type",a.document.type,r.errors);text(j,"id",a.document.id.value,r.errors);text(j,"name",a.document.name,r.errors);auto ns=j.find("nodes");if(ns==j.end()||!ns->is_array())error(r.errors,ErrorCode::invalid_asset,"nodes","expected an array");else for(const auto& x:*ns){if(!x.is_object()){error(r.errors,ErrorCode::invalid_asset,"nodes","expected objects");continue;}EntityNode n;text(x,"id",n.id,r.errors);text(x,"name",n.name,r.errors);auto p=x.find("parent");if(p!=x.end()){if(!p->is_string())error(r.errors,ErrorCode::invalid_asset,"parent","expected a string");else n.parent=p->get<std::string>();}transform_read(x,"transform",n.transform,r.errors);number(x,"zOrder",n.z_order,r.errors);auto d=x.find("drawable");if(d==x.end()||!d->is_object()){error(r.errors,ErrorCode::invalid_asset,"drawable","expected an object");}else{std::string k;text(*d,"kind",k,r.errors);if(k=="none")n.drawable.kind=EntityDrawableKind::none;else if(k=="vector")n.drawable.kind=EntityDrawableKind::vector;else if(k=="texture")n.drawable.kind=EntityDrawableKind::texture;else error(r.errors,ErrorCode::invalid_asset,"drawable.kind","unsupported drawable kind");ref_read(*d,"resource",n.drawable.resource,r.errors);ref_read(*d,"material",n.drawable.material,r.errors);}a.nodes.push_back(std::move(n));}if(!r.errors.empty())return r;auto v=validate_entity(m,a);if(!v.ok()){r.errors=std::move(v.errors);return r;}r.entity=std::move(a);return r;}
