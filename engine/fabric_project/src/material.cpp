...
ValidationReport validate_material(const ProjectManifest&, const MaterialDefinition& a) {
    ValidationReport r;
    if (a.document.schema_version != current_material_schema_version) error(r.errors, ErrorCode::unsupported_schema_version, "schemaVersion", "only material schema version 1 is supported");
