...
AnimationResult publish_animation(const std::filesystem::path& root,const ProjectManifest& m,const AnimationClip& a){AnimationResult r;auto migrated=a;if(migrated.document.schema_version==1)migrated.document.schema_version=current_animation_schema_version;auto v=validate_animation(m,migrated);if(!v.ok()){r.errors=std::move(v.errors);return r;}auto p=animation_document_path(m,migrated.document.id);auto s=save_document_atomic(root,p,serialize_animation(migrated),[&](std::string_view x){return parse_validation(m,x);});if(!s.ok()){r.errors=std::move(s.errors);return r;}return load_animation(root,m,p);}
