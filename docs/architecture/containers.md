...
C4Container
    ...
    System_Boundary(fabric, "Vertex Loom") {
        ...
        Container(map, "Map Studio", "C++20 / SDL2 / OpenGL / Dear ImGui", "Ouvre Map, Scene et Mechanics comme workspaces contextuels dans une seule surface documentée par EditorContext plutôt que comme fenêtres concurrentes ; le workspace Map conserve Layers gauche, canvas central et Inspector droit, compose et sauvegarde même sans renderer, conserve dans sa passerelle de preview les erreurs document, PNG, upload GPU et draw, expose un smoke d'accessibilité clavier/contraste avec capture non uniforme, puis ne prévisualise ou publie qu'après rendu valide")
