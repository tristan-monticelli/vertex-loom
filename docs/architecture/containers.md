...
C4Container
    ...
    System_Boundary(fabric, "Vertex Loom") {
        ...
        Container(map, "Map Studio", "C++20 / SDL2 / OpenGL / Dear ImGui", "Ouvre une map ou scène, choisit les références par pickers typés, compose instances/prefabs/collisions/triggers, édite les paramètres par overrides, administre les transitions de fermeture propre ou avec sauvegarde, rafraîchit le catalogue après chaque création, partage les champs de nom et recentre une seule fois le focus sur le premier champ invalide de chaque formulaire, expose un smoke d'accessibilité clavier/contraste, puis publie après preview")
        ...
        Container(projectlib, "fabric_project", "C++20 / nlohmann-json", "Contrats JSON stricts, dont profils shader textiles/plastiques, classification de textures, collisions configurables, BehaviorGraph v1 et EntityTransformation v1, registre et fermeture transitive des paquets")
