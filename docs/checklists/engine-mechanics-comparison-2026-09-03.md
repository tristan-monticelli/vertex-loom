...
| Animation | A13 — animation guidée par chemin | Le clip d'animation peut piloter la propriété `pathFollower.progress` sur l'instance ; le Preview Runtime échantillonne alors position+tangente sans modifier le chemin source, avec test publié | non | oui | oui | partiel | ✓ | ✓ | ✓ | partiel | partiel | ✓ | partiel | Le binding Runtime est fonctionnel, mais la création de piste dédiée et son affordance Studio restent à exposer | P1 | Ajouter une action “Animer la progression” dans l'Inspector PathFollower et une piste spécialisée dans la timeline | G2, U2, E2, S2 |
