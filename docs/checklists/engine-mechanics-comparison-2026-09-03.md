...
| Déplacement | A12 — rail de déplacement d'instance | `MapInstance.pathFollower` est éditable dans l'Inspector Map, le rail et le curseur sont visibles et repositionnables au clic dans le canvas, le Runtime l'évalue à chaque pas fixe ; le test recharge aussi le paquet exporté et vérifie le document de chemin ; les combinaisons prefab mécanique + rail sont refusées localement | oui | oui | oui | implémenté | ✓ | ✓ | ✓ | ✓ | ✓ | N/A | partiel | Le parcours création → réglage visuel → Preview → paquet est couvert ; il manque seulement le geste E2E qui configure le rail dans l'interface | P1 | Ajouter un scénario UI qui configure le rail puis recharge le paquet exact | G2, U2, E2, S2 |
| Animation | A13 — animation guidée par chemin | Le clip d'animation peut piloter la propriété `pathFollower.progress` sur l'instance ; le Preview Runtime échantillonne alors position+tangente sans modifier le chemin source, avec test publié ; la timeline expose `Path follower / Progress` sur le nœud racine ; l'Inspector Map peut créer et attacher un clip linéaire | non | oui | oui | partiel | ✓ | ✓ | ✓ | partiel | partiel | ✓ | partiel | Le parcours contextuel crée le clip et l'attache ; la preuve E2E graphique et l'ouverture automatique d'un timeline partagé restent à exposer | P1 | Ajouter un scénario UI publié et conserver l'instance comme contexte lors de l'ouverture du timeline unifié | G2, U2, E2, S2 |
...
| UX globale | A56 — registre d'actions et palette de commandes contextuelle | Save, Undo/Redo, visuels→Entity, Entity→Animation, ouverture Animation Graph, création+attachement Behavior et création d'animation PathFollower partagent registre, disponibilité et palette ; le picker Behavior reste visible en mode guidé | partiel | N/A | N/A | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | partiel | partiel | Le parcours visuel→Entity→Animation/Logic/PathFollower devient découvrable et cohérent, mais publication et les autres créations n'ont pas encore toutes un propriétaire d'action unique | P0 | Migrer ensuite les actions métier par parcours et exiger une preuve bouton + palette + raison de blocage | G16, U15, E17, GM8, C10, S5, R6 |
| Inspecteur | A57 — divulgation progressive, recherche, multi-édition, revert et favoris | Sections repliables et pickers ; Map Inspector permet maintenant la recherche locale des propriétés d'instance ; favoris, revert uniforme et multi-édition restent limités | partiel | N/A | N/A | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Les fonctions avancées restent longues à trouver et les écarts aux défauts sont peu visibles | P1 | Étendre la recherche aux propriétés Entity/Animation, puis ajouter propriétés modifiées, revert et édition commune de sélection | G9, U15, E9, GM8, C6, S5, R6 |
