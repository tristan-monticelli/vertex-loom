...
| Créer un clip d'animation | Entity→nouveau clip→première clé→auto-key→playhead→seconde pose→lecture/pause→correction de clé→événement→reload est prouvé par clics et drags UI | L3 | partiel | Le runtime publié n'est pas encore dans ce flux transversal | P0 | Étendre l'E2E jusqu'au paquet publié |
...
désormais 10 730 lignes et 1 365 appels
...
| Animation | A17 — clips, clés, interpolation, easing, segments, événements | `animation.hpp` v4 ; le flux UI transversal crée deux poses, lit/pause, déplace la seconde clé, ajoute un événement puis prouve les temps après reload ; `AnimationTimeline` déplace et scale atomiquement une sélection autour du playhead | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Création, lecture, correction, multi-clés et événement existent ; l'exécution publiée manque au même flux | P0 | Étendre le scénario UI jusqu'au runtime publié et son geste multi-clés | G3, U3, E3, GM1, S1, R1, G8, U8, E8, GM4, S4, R4 |
| Animation | A18 — machine à états d'animation | `animation_state_machine.hpp` ; le canevas choisit un clip, crée une carte sans ID puis relie les deux états par clics avant reload et capture OpenGL | oui | oui | oui | implémenté | ✓ | ✓ | ✓ | partiel | ✓ | partiel | ✓ | Ajout et connexion nominaux utilisables ; layout manuel, diagnostics d'arête et runtime publié non prouvés | P1 | Ajouter erreurs locales et test UI→Preview Runtime | G3, U3, E3, R1, R5 |
...
| UX Animation | A40 — workspace timeline, création de piste et keying contextuel | Le flux transversal crée le clip et deux poses, lit/pause, déplace le second losange, ajoute un événement et recharge par gestes UI ; box-select et `Alt`+glisser scalent atomiquement le groupe autour du playhead | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | partiel | ✓ | ✓ | Timeline utilisable avec édition groupée ; runtime publié et geste graphique de scale non encore prouvés | P0 | Ajouter la preuve publiée et le scale multi-clés au même E2E | G8, U8, E8, GM4, C7, S4, R4 |
...
| UX globale | A55 — sélection logique synchronisée entre arbre, canvas, timeline, graphe et inspecteur | Entity et Behavior résolvent leurs sélections stables entre canvas/listes/inspecteur ; Map, Scene et autres graphes gardent encore des états locaux | partiel | N/A | N/A | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Behavior ne dérive plus après mutation ; le propriétaire unique transversal reste incomplet | P0 | Étendre les IDs stables restants puis porter la sélection commune dans `EditorContext` | G9, U15, E9, GM8, C6, S5, R6 |
...
| Outils canvas | A59 — modes contextuels et manipulation directe des relations | Gizmos Entity/vectoriels, IK chaîne+cible, placement Map et déplacement Mechanics corps/pivots/capteurs ; le mesh possède une initialisation nominale valide, mais mesh, poids, XPBD et joints restent sans manipulation directe | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | IK, initialisation mesh et Mechanics de base sont prouvés, mais plusieurs systèmes avancés restent derrière des formulaires | P0 | Poignées mécaniques restantes, puis édition mesh/poids et Physics canvas | G6, U8, E18, GM9, C6, S6, R6 |
...
| Architecture UX | A61 — shell et workspaces modulaires testables | `map_studio/main.cpp` est descendu à 2 937 lignes après extraction de cinq modules ; Behavior, Animation et Rig/Physics Entity possèdent désormais leurs modules et états, mais `asset_studio/main.cpp` reste à 10 730 lignes après raccordement du nouveau parcours E2E | partiel | N/A | N/A | partiel | ✓ | ✓ | ✓ | non prouvé | partiel | non prouvé | non prouvé | La frontière Animation est complète et Rig/Physics est isolé, mais la hiérarchie/inspecteur Entity, Visual et les harnais E2E restent fortement couplés au shell Asset | P0 | Continuer tranche 3 : hiérarchie/inspecteur Entity puis Visual hors du bootstrap, mêmes sessions et E2E | G18, U17, E19, GM5, C11, S5, R6 |
