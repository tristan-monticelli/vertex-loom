...
| UX globale | A56 — registre d'actions et palette de commandes contextuelle | Save, Undo/Redo, `create_entity_from_visuals` et `animate_selection` partagent registre, disponibilité et palette ; création Entity partage aussi bouton et menu contextuel | partiel | N/A | N/A | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | partiel | partiel | Le parcours visuel→Entity→Animation devient découvrable et cohérent, mais graphes, publication et autres créations n'ont pas encore tous un propriétaire d'action unique | P0 | Migrer ensuite les actions métier par parcours et exiger une preuve bouton + palette + raison de blocage | G16, U15, E17, GM8, C10, S5, R6 |
