...
| Créer un clip d'animation | Entity→nouveau clip→première clé→auto-key→playhead→seconde pose→reload est prouvé par clics et drag UI | L3 | partiel | Lecture, correction de clé, événement et runtime publié ne sont pas encore dans ce flux transversal | P0 | Étendre l'E2E à lecture→édition de clé→événement→runtime |
| Éditer une machine à états | Canevas `Animation Graph`, cartes d'états, flèches et inspecteurs avancés | L3 | partiel | La connexion source→cible est prouvée par clics et reload ; ajout d'état, erreurs locales et layout manuel restent à couvrir | P0 | Ajouter palette/recherche d'état, erreurs sur les arêtes et preuve Preview Runtime |
| Construire un Behavior Graph | Canevas de cartes/ports/flèches avec connexion source→cible et inspecteur typé | L3 | partiel | Connexion et exécution sont prouvées ; ajout de nœud sans ID, recherche d'action, trace sur le graphe et breakpoint manquent | P0 | Ajouter palette recherchable, mise en évidence de la trace et breakpoints |
| Construire une mécanique | Canevas Map Studio avec cartes, ports, flèches et connexion compatible ; simulation et inspecteurs existants | L2/L3 | partiel | Le canevas est implémenté mais le parcours gestes UI→reload→simulation n'a pas encore sa preuve graphique dédiée | P0 | Ajouter l'E2E de connexion, puis handles body/pivot/joint directement sur la map |
...
| Animation | A17 — clips, clés, interpolation, easing, segments, événements | `animation.hpp` v4 ; le flux UI transversal crée le clip, une première clé et une seconde pose auto-key par drag, puis recharge | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Création et deux poses prouvées ; correction de clé, événement et exécution publiée manquent au même flux | P0 | Étendre le scénario UI à correction, événement, save/reload et runtime | G3, U3, E3, GM1, S1, R1, G8, U8, E8, GM4, S4, R4 |
| Animation | A18 — machine à états d'animation | `animation_state_machine.hpp` ; canevas avec cartes/flèches ; l'E2E relie deux états par clics puis vérifie le reload et une capture OpenGL | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | partiel | ✓ | partiel | ✓ | Connexion nominale utilisable ; création visuelle d'état, layout manuel, diagnostics d'arête et runtime publié non prouvés | P0 | Ajouter palette d'états, erreurs locales et test UI→Preview Runtime | G3, U3, E3, R1, R5 |
...
| UX Animation | A40 — workspace timeline, création de piste et keying contextuel | Le flux transversal crée clip, première clé et seconde pose auto-key par gestes UI ; l'ancien A→B reste préparé par API | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | partiel | ✓ | ✓ | Première animation A→B utilisable ; correction, événements et runtime publié non prouvés | P0 | Ajouter lecture, correction de clé, événement et preuve publiée au même E2E | G8, U8, E8, GM4, C7, S4, R4 |
...
| Comportement | A23 — BehaviorGraph générique signaux/actions | `behavior_graph.hpp` ; canevas cartes/ports/flèches ; E2E relie deux nœuds par clics, recharge puis exécute le signal | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | partiel | ✓ | N/A | ✓ | Connexion nominale utilisable ; création demande encore un ID et le debug visuel manque | P0 | Palette recherchable sans ID, trace superposée et breakpoints | E10, C8, R5 |
...
| Mécaniques | A27 — nœuds body/pivot/joint/motor | `MechanicNodeKind`, compilateur/presets ; canevas cartes/ports/flèches et connexion rapide dans Map Studio | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | ✓ | N/A | N/A | Graphe lisible et connectable, mais E2E graphique et handles physiques sur la map non prouvés | P0 | Prouver UI→reload→simulation puis ajouter handles body/pivot/joint | G5, U4, E4, GM2, C4 |
