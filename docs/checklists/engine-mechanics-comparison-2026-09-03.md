...
| Construire une mécanique | Instance Map sélectionnée→action contextuelle→preview paramétrée ; graphe et canevas spatial côte à côte ; déplacement, taille, rotation et joint directs ; retour Map avec overlay borné à l'instance et paramètres spatiaux manipulables ; E2E navigation+gestes→reload→package→runtime | L4 | utilisable | Les paramètres publiés sont directs et undoables ; les propriétés privées exigent encore l'ouverture de Mechanics | P1 | Ajouter un raccourci contextuel depuis une forme en lecture seule vers son nœud Mechanics |
...
ImGui ; `map_studio/main.cpp` contient 3 289 lignes et 422 appels ImGui. Le
...
| Mécaniques | A27 — nœuds body/pivot/joint/motor | `MechanicNodeKind`, compilateur/presets ; l'instance Map sélectionnée ouvre sa preview avec overrides ; déplacement, taille, rotation et couronne de joint ; l'overlay exact expose les paramètres spatiaux du prefab par gestes undoables ; E2E recharge, simule, publie et lance le paquet | oui | oui | oui | implémenté | ✓ | ✓ | ✓ | ✓ | ✓ | N/A | N/A | Navigation, authoring et paramètres d'overlay sont directs ; une propriété non publiée reste volontairement protégée | P1 | Ouvrir le nœud Mechanics ciblé depuis une forme protégée | G5, U4, E4, GM2, C4 |
...
| Architecture UX | A61 — shell et workspaces modulaires testables | `map_studio/main.cpp` est à 3 289 lignes ; interaction spatiale Mechanics, canvas Map, Behavior, Animation, preuve de publication Animation et Rig/Physics Entity possèdent leurs modules, mais `asset_studio/main.cpp` reste à 10 795 lignes | partiel | N/A | N/A | partiel | ✓ | ✓ | ✓ | non prouvé | partiel | non prouvé | non prouvé | L'overlay Map réutilise le module Mechanics, mais hiérarchie/inspecteur Entity, Visual et plusieurs harnais restent couplés au shell Asset | P0 | Continuer tranche 3 : hiérarchie/inspecteur Entity puis Visual hors du bootstrap | G18, U17, E19, GM5, C11, S5, R6 |
