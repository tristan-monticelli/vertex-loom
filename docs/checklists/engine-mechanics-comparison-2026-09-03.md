...
| Animation | A13 — animation guidée par chemin | Le clip d'animation peut piloter la propriété `pathFollower.progress` sur l'instance ; le Preview Runtime échantillonne alors position+tangente sans modifier le chemin source, avec test publié ; la timeline expose `Path follower / Progress` sur le nœud racine | non | oui | oui | partiel | ✓ | ✓ | ✓ | partiel | partiel | ✓ | partiel | Le binding et sa découverte dans Asset Studio sont fonctionnels ; l'action contextuelle depuis l'Inspector PathFollower et la preuve UI dédiée restent à exposer | P1 | Ajouter une action “Animer la progression” qui ouvre/crée le clip et conserve l'instance comme contexte | G2, U2, E2, S2 |
