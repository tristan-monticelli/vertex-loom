...
| Déplacement | A12 — rail de déplacement d'instance | `MapInstance.pathFollower` est éditable dans l'Inspector Map, le rail et le curseur sont visibles dans le canvas, le Runtime l'évalue à chaque pas fixe ; le test recharge aussi le paquet exporté et vérifie le document de chemin ; les combinaisons prefab mécanique + rail sont refusées localement | oui | oui | oui | implémenté | ✓ | ✓ | ✓ | ✓ | ✓ | N/A | partiel | Le parcours création → réglage visuel → Preview → paquet est couvert ; il manque seulement le geste E2E qui configure le rail dans l'interface | P1 | Ajouter un scénario UI qui configure le rail puis recharge le paquet exact | G2, U2, E2, S2 |
