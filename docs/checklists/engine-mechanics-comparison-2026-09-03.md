...
| --- | --- | --- | --- | --- | --- | --- |
| Créer une Entity depuis un visuel | Un visuel sélectionné expose désormais `Create Entity from visual` et crée directement l'Entity reliée ; la multi-sélection n'existe pas encore | Le premier résultat mono-visuel est direct ; composer plusieurs sources demande encore des ajouts successifs | Godot/Unity/Construct créent ou composent depuis hiérarchie, projet ou canvas ; GameMaker accepte le drag d'asset | partiel | Étendre la sélection à plusieurs visuels et créer tous les nœuds en une commande | P1 |
...
| Lire et corriger une animation | Dock Timeline redimensionnable sous le canvas : transport, playhead, zoom, lignes typées, marqueurs, sélection additive et drag des clés | La sélection rectangle et l'édition de courbe restent absentes | Tous les outils d'animation comparés exposent pistes + playhead + clés dans une timeline dédiée | partiel | Ajouter sélection rectangle et vue courbe sans réintroduire un formulaire obligatoire | P1 |
...
| UX Entity | A39 — composition contextuelle, arbre et édition directe | Action mono-visuel→Entity directe, boutons/drop d'arbre et gizmo translation ; ADR-0130/0147 | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | partiel | ✓ | partiel | partiel | La création mono-visuel est directe ; multi-sélection et arbre dédié restent à fermer | P1 | Ajouter sélection multiple et arbre synchronisé canvas/inspecteur | G8, U8, E8, GM4, C6, S4, R4 |
| UX Animation | A40 — workspace timeline, création de piste et keying contextuel | Dock Timeline réel + `Key Rotation` cliqué en E2E, piste créée, sauvegardée et rechargée ; ADR-0147 | partiel | oui | oui | partiel | ✓ | ✓ | ✓ | ✓ | partiel | ✓ | ✓ | Timeline/key rapide livrés ; auto-key par gizmo, sélection rectangle et courbes restent ouverts | P1 | Relier les gizmos à auto-key puis ajouter sélection rectangle et courbes | G8, U8, E8, GM4, C6, S4, R4 |
