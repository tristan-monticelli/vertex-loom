...
| Construire une mécanique | Instance Map sélectionnée→action contextuelle→preview paramétrée ; graphe et canevas spatial côte à côte ; déplacement, taille, rotation et joint directs ; E2E navigation+connexion+4 glissers→reload→package→runtime | L4 | utilisable | Le parcours principal et l'accès depuis la map sont prouvés ; l'overlay mécanique dans le canvas Map manque | P1 | Projeter les poignées de la mécanique sur l'instance dans le canvas Map |
...
ImGui ; `map_studio/main.cpp` contient 3 172 lignes et 425 appels ImGui. Le
...
| Mécaniques | A27 — nœuds body/pivot/joint/motor | `MechanicNodeKind`, compilateur/presets ; l'instance Map sélectionnée ouvre sa preview avec overrides ; déplacement, taille, rotation et couronne de joint ; E2E reconnecte, recharge, simule, publie et lance le paquet | oui | oui | oui | implémenté | ✓ | ✓ | ✓ | ✓ | ✓ | N/A | N/A | Navigation instance→Mechanics et authoring sont prouvés ; l'overlay reste séparé du canvas Map | P1 | Projeter les poignées Mechanics dans le contexte de l'instance Map | G5, U4, E4, GM2, C4 |
