...
bool json_equal(const nlohmann::json& left, const nlohmann::json& right) {
    if (left.is_number() && right.is_number()) {
        return std::abs(left.get<double>() - right.get<double>()) <= 1.0e-5;
    }
    if (left.type() != right.type()) return false;
    if (left.is_object()) {
        if (left.size() != right.size()) return false;
        for (const auto& [key, value] : left.items()) {
            if (!right.contains(key) || !json_equal(value, right.at(key)))
                return false;
        }
        return true;
    }
    if (left.is_array()) {
        ...
        for (std::size_t index = 0; index < left.size(); ++index) {
            if (!json_equal(left[index], right[index])) return false;
        }
        return true;
    }
    return left == right;
...
bool files_equal(const std::filesystem::path& left,
    ...
    if (left.extension() == ".json")
        return json_equal(read_json(left), read_json(right));
...
TEST_CASE("textile head fixture is composed and cropped through Studio") {
    ...
    CHECK(std::get<float>(beam_evaluation->properties.front().value) ==
          Catch::Approx(2.0F));
    REQUIRE(runtime.run());
    REQUIRE(runtime.last_frame_packets().size() == resolved.packets.size() + 2U);
